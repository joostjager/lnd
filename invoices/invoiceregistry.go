...
func (i *InvoiceRegistry) AddInvoice(invoice *channeldb.Invoice,
	paymentHash lnhash.Hash) (uint64, error) {
...
func (i *InvoiceRegistry) SettleInvoice(rHash lnhash.Hash,
	...
	if err == channeldb.ErrInvoiceAlreadySettled {
		log.Debugf("Invoice %x already settled", rHash[:])
		return nil
	}
...
// payment hash.
func (i *InvoiceRegistry) CancelInvoice(payHash lnhash.Hash) error {
	i.Lock()
	defer i.Unlock()
...
func (i *InvoiceRegistry) notifyClients(hash lnhash.Hash,
	invoice *channeldb.Invoice,
	state channeldb.ContractState) {
...
func (i *InvoiceRegistry) SubscribeSingleInvoice(
	hash lnhash.Hash) *SingleInvoiceSubscription {
