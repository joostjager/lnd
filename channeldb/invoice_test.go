...
func TestInvoiceWorkflow(t *testing.T) {
	...
	// hash.
	if _, err := db.AddInvoice(fakeInvoice, fakeInvoice.Terms.PaymentPreimage); err != nil {
	...
	if _, err := db.AddInvoice(fakeInvoice, fakeInvoice.Terms.PaymentPreimage); err != ErrDuplicateInvoice {
	...
	for i := 1; i < len(invoices)-1; i++ {
		...
		if _, err := db.AddInvoice(invoice, invoice.Terms.PaymentPreimage); err != nil {
