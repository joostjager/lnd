...
func fetchPayment(bucket *bbolt.Bucket) (*MPPayment, error) {
	...
	var htlcs map[uint64]*HTLCAttempt
...
// the given bucket.
func fetchHTLCAttempts(bucket *bbolt.Bucket) (map[uint64]*HTLCAttempt, error) {
	...
	err := bucket.ForEach(func(k, _ []byte) error {
		...
		wireInfo, err := fetchHTLCHTLCWireInfo(
			htlcBucket,
		)
		if err != nil {
			return err
...
// fetchHTLCHTLCWireInfo fetches the payment attempt for this HTLC from
// the bucket.
func fetchHTLCHTLCWireInfo(bucket *bbolt.Bucket) (*HTLCWireInfo, error) {
	...
	r := bytes.NewReader(b)
	return deserializeHTLCWireInfo(r)
}
...
func serializeHTLCWireInfo(w io.Writer, a *HTLCWireInfo) error {
...
func deserializeHTLCWireInfo(r io.Reader) (*HTLCWireInfo, error) {
	var a = &HTLCWireInfo{}
