...
// PaymentAddress is an identifier communicated through the invoice that the
// sender uses to authorize the payment.
type PaymentAddress [32]byte
...
type Invoice struct {
	...
	// PaymentAddr is an identifier communicated through the invoice that
	// the sender uses to authorize the payment.
...
func serializeHtlcs(w io.Writer, htlcs map[CircuitKey]*InvoiceHTLC) error {
	for key, htlc := range htlcs {
		...
		tlvStream, err := tlv.NewStream(
			...
			tlv.MakePrimitiveRecord(mppTotalAmtType, &totalAmt),
		)
		if err != nil {
			return err
		}
