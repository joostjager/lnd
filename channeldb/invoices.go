...
var (
	...
	//
	//   settleIndexNo => invoiceKey
	acceptedIndexBucket = []byte("invoice-accepted-index")
)
...
const (
	// ContractOpen means the invoice only been created.
	ContractOpen = 0

	// ContractSettled means the invoice has been paid.
	ContractSettled = 1
...
type ContractTerm struct {
	...
	// State indicates if this particular contract term has been fully
...
type Invoice struct {
	...
	// AcceptIndex is an auto-incrementing integer that acts as a
	// monotonically increasing sequence number for all accepted invoices.
	// Clients can then use this field as a "checkpoint" of sorts when
	// implementing a streaming RPC to notify consumers of instances where
	// an invoice has been settled before they re-connected.
	//
...
func (d *DB) AcceptInvoice(paymentHash [32]byte,
	...
	err := d.Update(func(tx *bbolt.Tx) error {
		...
		invoice, err := acceptInvoice(
			invoices, acceptedIndex, invoiceNum, amtPaid,
		)
		if err != nil {
			return err
		}
...
// NOTE: The index starts from 1, as a result. We enforce that specifying a
// value below the starting index value is a noop.
func (d *DB) InvoicesAcceptedSince(sinceAcceptIndex uint64) ([]Invoice, error) {
	var acceptedInvoices []Invoice

	// If an index of zero was specified, then in order to maintain
	// backwards compat, we won't send out any new invoices.
	if sinceAcceptIndex == 0 {
		return acceptedInvoices, nil
	}

	var startIndex [8]byte
	byteOrder.PutUint64(startIndex[:], sinceAcceptIndex)
	...
	err := d.DB.View(func(tx *bbolt.Tx) error {
		...
		acceptIndex := invoices.Bucket(acceptedIndexBucket)
		if acceptIndex == nil {
			return ErrNoInvoicesCreated
		}

		// We'll now run through each entry in the accept index starting
		// at our starting index. We'll continue until we reach the
		// very end of the current key space.
		invoiceCursor := acceptIndex.Cursor()

		// We'll seek to the starting index, then manually advance the
		// cursor in order to skip the entry with the since accept index.
		invoiceCursor.Seek(startIndex[:])
		...
		for ; seqNo != nil && bytes.Compare(seqNo, startIndex[:]) > 0; seqNo, invoiceKey = invoiceCursor.Next() {
			...
			acceptedInvoices = append(acceptedInvoices, invoice)
		}
...
func acceptInvoice(invoices, acceptedIndex *bbolt.Bucket, invoiceNum []byte,
	amtPaid lnwire.MilliSatoshi) (*Invoice, error) {
	...
	// update the settle index so we can place this settle event in the
	// proper location within our time series.
	nextSettleSeqNo, err := acceptedIndex.NextSequence()
	if err != nil {
		return nil, err
	}
	...
	var seqNoBytes [8]byte
	byteOrder.PutUint64(seqNoBytes[:], nextSettleSeqNo)
	if err := acceptedIndex.Put(seqNoBytes[:], invoiceNum); err != nil {
		return nil, err
	}

	invoice.AmtPaid = amtPaid
	invoice.Terms.State = ContractAccepted
	invoice.AcceptDate = time.Now()
	invoice.AcceptIndex = nextSettleSeqNo
