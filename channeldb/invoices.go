...
func (d *DB) UpdateInvoice(paymentHash lntypes.Hash,
	...
	err := d.Update(func(tx *bbolt.Tx) error {
		...
		settledInvoice, err = acceptOrSettleInvoice(
			invoices, settleIndex, invoiceNum, updateInvoice,
		)
...
func acceptOrSettleInvoice(invoices, settleIndex *bbolt.Bucket,
	invoiceNum []byte, updateInvoice InvoiceUpdateCallback) (
	*Invoice, error) {
	...
	// callback.
	//
	// TODO(joostjager): Make it really a deep copy including slices and
	// maps.
	copy := invoice
	update, err := updateInvoice(&copy)
	if err != nil {
		return &invoice, err
	}
