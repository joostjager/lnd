...
type InvoiceDatabase interface {
	...
	LookupInvoice(lnhash.Hash) (channeldb.Invoice, uint32, error)

	// SettleInvoice attempts to mark an invoice corresponding to the
	// passed payment hash as fully settled.
	SettleInvoice(payHash lnhash.Hash, paidAmount lnwire.MilliSatoshi,
		preimage *lnhash.Hash) error

	// CancelInvoice attempts to cancel the invoice corresponding to the
	// passed payment hash.
	CancelInvoice(payHash lnhash.Hash) error

	// AcceptInvoice attempts to mark an invoice corresponding to the
	// passed payment hash as accepted.
	AcceptInvoice(payHash lnhash.Hash, paidAmount lnwire.MilliSatoshi) error
}
