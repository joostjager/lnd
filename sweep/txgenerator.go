...
func newTxGenerator(relayFeePerKW lnwallet.SatPerKWeight) *TxGenerator {
	dustLimit := txrules.GetDustThreshold(
		lnwallet.P2WPKHSize,
		btcutil.Amount(relayFeePerKW.FeePerKVByte()))

	return &TxGenerator{
		dustLimit: int64(dustLimit),
...
func (s *TxGenerator) Generate(sweepableInputs []Input,
	satPerKW lnwallet.SatPerKWeight) (
	[][]Input, error) {
	...
	for len(sweepableInputs) > 0 && !stopSweep {
		...
		txes = append(txes, inputList)
	}

	return txes, nil
}

func (s *TxGenerator) createSweepTx(inputs []Input, outputPkScript []byte,
