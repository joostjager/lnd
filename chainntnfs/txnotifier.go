...
func (c ConfRequest) String() string {
	if c.TxID != ZeroHash {
		return fmt.Sprintf("txid=%v", c.TxID)
	}
	return fmt.Sprintf("script=%v", c.PkScript)
}
...
func (c SpendRequest) String() string {
	if c.OutPoint != ZeroOutPoint {
		return fmt.Sprintf("outpoint=%v", c.OutPoint)
	}
	return fmt.Sprintf("script=%v", c.PkScript)
}
...
func (n *TxNotifier) filterTx(tx *btcutil.Tx, blockHash *chainhash.Hash,
	...
	txHash := tx.Hash()
	if onSpend != nil {
		// constructDetails is a helper closure that will construct the
		// spend details of a request and hand them off to the onSpend
		// callback.
		constructDetails := func(spendRequest SpendRequest,
		...
		for i, txIn := range tx.MsgTx().TxIn {
			...
			if _, ok := n.spendNotifications[spendRequest]; ok {
				constructDetails(spendRequest, prevOut, uint32(i))
			}
	...
	if onConf != nil {
		...
		// onConf callback.
		constructDetails := func(confRequest ConfRequest) {
...
// meeting each requests' desired number of confirmations for all current and
