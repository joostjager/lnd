...
func (b *BitcoindNotifier) confDetailsManually(confRequest chainntnfs.ConfRequest,
	...
	for height := currentHeight; height >= heightHint && height > 0; height-- {
		...
		for txIndex, tx := range block.Transactions {
			switch {
			// If the request has a zero hash, then we should match
			// on the output script instead, so we'll do so.
			case confRequest.TxID == chainntnfs.ZeroHash:
				matched := false
				pkScript := confRequest.PkScript.Script()
			...
			// Otherwise, we should match on the txid as usual.
			case tx.TxHash() == confRequest.TxID:
				break
			...
			// If we've reached here, then either the txid or output
	...
	for height := endHeight; height >= startHeight && height > 0; height-- {
		...
		for _, tx := range block.Transactions {
			for i, txIn := range tx.TxIn {
				switch {
				// If the request has a zero outpoint, then we
				// should match on the output script instead, so
				// we'll do so.
				case spendRequest.OutPoint == chainntnfs.ZeroOutPoint:
					...
					got := pkScript.Script()
					expected := spendRequest.PkScript.Script()
				...
				// Otherwise, we should match on the outpoint
				// being spent as usual.
				case txIn.PreviousOutPoint == spendRequest.OutPoint:
					break
				...
				// If we've reached here, then either the
				// outpoint or output script was spent, so we'll
				// construct its spend details and hand them
				// over to the TxNotifier so that it can
				// properly notify its registered clients.
				txHash := tx.TxHash()
...
// for the current tip of the chain.
