...
func CreateRPCInvoice(invoice *channeldb.Invoice,
	activeNetParams *chaincfg.Params) (*Invoice, error) {
	...
	var state Invoice_InvoiceState
...
func CreateRPCRouteHints(routeHints [][]routing.HopHint) []*RouteHint {
	var res []*RouteHint

	for _, route := range routeHints {
		hopHints := make([]*HopHint, 0, len(route))
		...
		routeHint := &RouteHint{HopHints: hopHints}
