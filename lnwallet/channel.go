...
func (lc *LightningChannel) ProcessChanSyncMsg(
	...
	// If we have a valid commitment point to check, then we'll verify it
	// here. We do this unconditionally, as even those with the new
	// tweakless format need a way to signal that they need the channel
	// closed at state zero.
	if commitPoint != nil && !commitPoint.IsEqual(msg.LocalUnrevokedCommitPoint) {
