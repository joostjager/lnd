...
type ControlTower interface {
	...
	//
	// TODO: should it check that the total value is not exceeded? Use case
	// is send to route where too much is sent.
...
type PaymentResult struct {
	...
	// the payment.
	HTLCs map[uint64]*channeldb.HTLCAttempt
}
...
func createSuccessResult(
	htlcs map[uint64]*channeldb.HTLCAttempt) *PaymentResult {
...
func createFailedResult(htlcs map[uint64]*channeldb.HTLCAttempt,
